...
class Evenimente extends BP_Controller
{
    ...
    public function index()
    {
        ...
        if (!$ret->NotLogged) {
            ...
            $sql          = new TableSql("SELECT t1.* FROM blog t1 WHERE Tip != 'blog'  ");


            $sql->addIfNeeded($filters['titlu'], '', " AND t1.titlu LIKE '%%%s%%' ");


            $sql->addIfNeeded($filters['Tip'], '', " AND t1.Tip = '%s' ");
    ...
    public function save()
    {
        ...
        if (!$ret->NotLogged) {
            ...
            if ($mode == "add") {
                ...
                $obj->DataCreare = date('Y-m-d H:i:s');
